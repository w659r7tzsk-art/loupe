// Delay initialization until DOM is ready and guard against missing globals to avoid runtime errors.
// Wrapped original script with safety checks and made hero/custom rings reachable via window for UI handlers.

if (typeof window !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => {
    if (typeof THREE === 'undefined') {
      console.error('Three.js is required for this page to work. Make sure the <script src="https://cdn.jsdelivr.net/npm/three@0.162.0/build/three.min.js"></script> is included before this script.');
      return;
    }
    ...
    function setRingLook(group, cut, metalKey) {
        ...
        const { band } = group.userData;
        band.material = metalMaterial(metalKey);
        group.children.forEach(c => {
            if (c.geometry && c.geometry.type && c.geometry.type.indexOf('Cylinder') !== -1 && c !== group.userData.gem) {
                c.material = band.material;
            }
        });
    ...
    if (heroCanvas) {
      ...
      if (stageEl) stageEl.classList.add('ready');
      let heroIntro = 0;

      // expose for UI
      window.heroRing = heroRing;
      window.heroFinger = heroFinger;
      ...
      function heroLoop(t) {
          ...
          if (heroFinger.userData && heroFinger.userData.mat) heroFinger.userData.mat.opacity = curFingerOpacity;
          if (heroFinger.userData && heroFinger.userData.nailMat) heroFinger.userData.nailMat.opacity = curFingerOpacity;
    ...
    if (customCanvas && customViewer) {
      ...
      customViewer.addEventListener('pointerdown', e => {
          dragging = true; lastX = e.clientX; lastY = e.clientY; if (customViewer.setPointerCapture) customViewer.setPointerCapture(e.pointerId);
          const hint = document.querySelector('.drag-hint'); if (hint) hint.classList.add('hidden');
      });
    ...
    if (metalSwatches) {
      Object.keys(METALS).forEach(k => {
          ...
          d.onclick = () => {
              ...
              try { setRingLook(window.heroRing, currentCut, currentMetal); } catch (e) {}
              try { setRingLook(window.customRing, currentCut, currentMetal); } catch (e) {}
          };
        ...
        function tick() {
            ...
            if (i <= word.length) {
                ...
                setTimeout(() => {
                    ...
                    if (loaderEl) loaderEl.classList.add('open');
                    setTimeout(() => { if (loaderEl) loaderEl.classList.add('hide'); }, 1250);
                }, 500);
            }
        }
